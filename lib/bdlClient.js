...
export async function getAdvancedStats(playerId, season = getCurrentNBASeason()) {
  try {
    const apiKey = process.env.BALLDONTLIE_API_KEY;
    console.log('Getting advanced stats:', {
      playerId,
      season,
      hasApiKey: !!apiKey,
      keyLength: apiKey?.length,
    ...
    const url = `https://api.balldontlie.io/v1/stats/advanced?player_ids[]=${playerId}&seasons[]=${season}&per_page=100`;
    console.log('Advanced stats request:', {
      url,
      headers: {
    ...
    console.log('Advanced stats data:', {
      ...
      firstPlayer: data?.data?.[0]?.player_id,
      netRating: data?.data?.[0]?.net_rating,
      sampleData: data?.data?.[0] ? {
        player_id: data.data[0].player_id,
        game_id: data.data[0].game_id,
        net_rating: data.data[0].net_rating,
        game: data.data[0].game
      } : null,
      allNetRatings: data?.data?.map(s => ({
        game_id: s.game_id,
        net_rating: s.net_rating,
        is_playoff: s.game?.postseason
      }))
    });

    if (!data?.data) {
      console.error('Invalid advanced stats response:', data);
